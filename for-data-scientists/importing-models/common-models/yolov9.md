...
This guide shows how to export a **trained** Yolov9 model to ONNX that can be directly uploaded and deployed on servers.

We'll be using the YOLOv9c as an example to show how any Yolov9-based model can be exported and trasformed into an ONNX that's compatible with the Nx AI Manager.
...
1. First, we need to clone the Yolov9 as it has the Python script that we need to convert the model form PyTorch to ONNX.

```bash
bash export-to-onnx.sh
```
